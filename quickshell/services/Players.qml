...
// around that by remembering the last user-set value and re-applying it
...
QtObject {
    ...
    // When the track changes, Tidal resets volume. The reset arrives slightly
    // after `trackTitleChanged`, so we restore via a short Timer (100 ms) to
    // overwrite the reset rather than be overwritten by it.
